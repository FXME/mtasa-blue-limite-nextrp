        ...
        for (; iter != m_Entities.end(); ++iter)
        {
            ...
            switch (ucEntityTypeID)
            {
                ...
                case CElement::VEHICLE:
                {
                    ...
                    //
                    // --slush
                    // Поменял и тут unsigned char на unsigned short
                    // Гильдоновские костыли
                    printf("[SYNC-DEBUG] Vehicle ID:%hd \n", pVehicle->GetModel());
                    BitStream.Write(static_cast<unsigned short>(pVehicle->GetModel()));
