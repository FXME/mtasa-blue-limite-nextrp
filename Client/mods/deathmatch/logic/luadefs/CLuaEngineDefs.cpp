...
std::uint32_t EngineStreamingGetMaxMemory()
{
    return g_pGame->GetStreaming()->GetMaxMemory();
}
...
void CLuaEngineDefs::LoadFunctions()
{
    constexpr static const std::pair<const char*, lua_CFunction> functions[]{
        ...
        {"engineRestreamMap", ArgumentParser<EngineRestreamMap>},

        {"engineStreamingFreeUpMemory", ArgumentParser<EngineStreamingFreeUpMemory>},
        {"engineStreamingGetMaxMemory", ArgumentParser<EngineStreamingGetMaxMemory>},
...
bool CLuaEngineDefs::EngineRestreamModel(unsigned short strModelId)
{
    g_pClientGame->RestreamModel(strModelId);
    return true;
}

bool CLuaEngineDefs::EngineRestreamMap(lua_State* const luaVM)
{
    g_pClientGame->RestreamMap();
    return true;
}
