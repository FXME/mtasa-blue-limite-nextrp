            ...
            bitStream.ReadCompressed(usDimension) && bitStream.ReadBit(bIsAttached))
        {
            ...
            switch (ucEntityTypeID)
            {
                ...
                case CClientGame::VEHICLE:
                {
                    ...
                    if (!(usModel >= 400 && usModel <= 611 || usModel >= 6526 && usModel <= 6711))
                    {
                        // SString strLine2("Ошибка протокола 39 , usModel2: %d ", usModel2;
                        SString strLine("Ошибка протокола 39 , айди тачки %d (до изменения)", usModel);
                        // WriteDebugEvent(strLine2);
                        WriteDebugEvent(strLine);

                        if (usModel + 6144 >= 6526 && usModel + 6144 <= 6711)
                        {
                            usModel = usModel + 6144;

                            SString strLine("Ошибка протокола 39 , айди тачки %d (после изменения)", usModel);
                            WriteDebugEvent(strLine);
                        }
                        else
                        {
                            SString strLine("Ошибка протокола 39 , айди тачки %d (Я СДОХЛА И ВЫЛЕТЕЛА)", usModel);
